...
class EncuestaModelo {
    ...
    /**
     * 1. Llenado de la Tabla Principal y Buscador
     */
    ...
    /**
     * 2. Datos para KPIs y Gráficas del Dashboard
     */
    ...
    public function getConteoActividades() {
        $this->db->query("SELECT actividad_principal, COUNT(*) as total FROM encuestas GROUP BY actividad_principal");
        return $this->db->resultSet();
    }
    ...
    public function getTendenciaDiaria() {
        $this->db->query("SELECT DATE(fecha_inicio) as fecha, COUNT(*) as total FROM encuestas GROUP BY DATE(fecha_inicio) ORDER BY fecha ASC");
        return $this->db->resultSet();
    }

    /**
     * 3. Obtener registro individual
     */
    ...
    /**
     * 4. ACTUALIZACIÓN INTEGRAL (Sincronizada con Captura.php)
     * Resuelve los errores de "Undefined array key" usando los nombres correctos
     */
    public function actualizarExpediente($data) {
        try {
            ...
            // Bindeo de Identidad (Sincronizado con Controlador)
            ...
            // Bindeo de Ubicación y Contacto
            ...
            // Bindeo de Datos Técnicos y Producción
