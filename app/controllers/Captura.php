...
class Captura extends Controller {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        // --- SECCIÓN NUEVA: ELIMINACIÓN DE ARCHIVOS MARCADOS CON LA "X" ---
        $documentosABorrar = [
            'solicitud'   => 'SOLICITUD',
            'identidad'   => 'IDENTIDAD',
            'domicilio'   => 'DOMICILIO',
        ...
        foreach ($documentosABorrar as $key => $prefijo) {
            // Si desde el JS se mandó el campo oculto delete_... en 1
            if (isset($_POST['delete_' . $key]) && $_POST['delete_' . $key] == '1') {
                // Buscamos cualquier extensión (.pdf, .jpg, .png, .zip) con ese prefijo
                $patron = $rutaBase . '/' . $folioCarpeta . '_' . $prefijo . '.*';
                $archivosEncontrados = glob($patron);
                
                if ($archivosEncontrados) {
                    foreach ($archivosEncontrados as $archivoFisico) {
                        if (file_exists($archivoFisico)) {
                            @unlink($archivoFisico); // Borrado físico del servidor
                        }
                    }
                }
            }
        }
        // --- FIN SECCIÓN DE ELIMINACIÓN ---

        // 3. CREACIÓN DE CARPETA (Si no existe)
        if (!is_dir($rutaBase)) {
        ...
        // 4. PROCESAMIENTO DE NUEVOS ARCHIVOS (Carga/Sobreescritura)
        $mapeoArchivos = [
        ...
        // 5. PREPARACIÓN DE DATOS (Todo a MAYÚSCULAS)
        $data = [
            'id'                => $id,
        ...
        // 6. EJECUTAR ACTUALIZACIÓN EN BASE DE DATOS
        if ($this->encuestaModel->actualizarExpediente($data)) {
            echo json_encode(['status' => 'success', 'msg' => '¡Expediente y documentos actualizados correctamente!']);
        } else {
