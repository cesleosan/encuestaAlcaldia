...
class Captura extends Controller {
    ...
    public function actualizar() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // 1. GESTIÓN DE ARCHIVOS FÍSICOS Y BITS DE COTEJO
            $mapeoDoc = [
            ...
            foreach ($mapeoDoc as $key => $info) {
                ...
                if (isset($_POST['delete_' . $key]) && $_POST['delete_' . $key] == '1') {
                    $viejos = glob($rutaBase . '/' . $folioCarpeta . '_' . $prefijo . '.*');
                    foreach ($viejos as $f) { @unlink($f); }
                    $dbChecks[$columna] = 0;
                } 
                elseif (isset($_FILES['file_' . $key]) && $_FILES['file_' . $key]['error'] === UPLOAD_ERR_OK) {
                    ...
                    $destino = $rutaBase . "/" . $folioCarpeta . "_" . $prefijo . "." . $ext;
            ...
            // 2. PREPARACIÓN DE DATA (MAPEO EXACTO A COLUMNAS DE MARIADB)
            $data = [
                'id'                => $id,
                ...
                'escolaridad'       => $_POST['grado_estudios'] ?? '', // DB: escolaridad
                ...
                'calle'             => mb_strtoupper($_POST['calle_numero'], 'UTF-8'), // DB: calle
                'colonia_nombre'    => mb_strtoupper($_POST['pueblo_colonia'], 'UTF-8'), // DB: colonia_nombre
                'codigo_postal'     => $_POST['cp'] ?? '', // DB: codigo_postal
                'tel_particular'    => $_POST['tel_particular'] ?? '',
                'tel_casa'          => $_POST['tel_casa'] ?? '',
                'tel_familiar'      => $_POST['tel_recados'] ?? '', // DB: tel_familiar
                'linea_ayuda'       => $_POST['tipo_produccion'] ?? 'AGRICOLA', // DB: linea_ayuda
                'registro_siniiga'  => $_POST['siniiga_status'] ?? 'NO', // DB: registro_siniiga
                'num_total_predios' => $_POST['num_total_predios'] ?? 1,
                'superficie_total'  => $_POST['superficie_prod'] ?? 0, // DB: superficie_total
                ...
                // Fases y Cotejos
                'fase_proceso'      => ($registro->fase_proceso == 'EMPADRONADO') ? 'SOLICITUD_INGRESADA' : ($_POST['fase_proceso'] ?? $registro->fase_proceso),
